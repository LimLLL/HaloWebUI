...
async def _prepare_openai_native_file_inputs(
    ...
    model_info = Models.get_model_by_id(requested_model_id) if requested_model_id else None
    if model_info and model_info.base_model_id:
        effective_model_id = str(model_info.base_model_id).strip()
    elif not effective_model_id:
        effective_model_id = str((model or {}).get("id") or "").strip()
        if not effective_model_id:
            effective_model_id = str((model or {}).get("original_id") or "").strip()
        if not effective_model_id and isinstance((model or {}).get("info"), dict):
            effective_model_id = str((model.get("info") or {}).get("base_model_id") or "").strip()
    ...
    if not str(upload_model_id or "").strip():
